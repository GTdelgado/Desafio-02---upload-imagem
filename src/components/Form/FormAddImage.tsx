...
export function FormAddImage({ closeModal }: FormAddImageProps): JSX.Element {
  ...
  const mutation = useMutation(
    (data: ForAddProps) => api.post('/api/images', data),
    {
      onSuccess: () => {
  ...
  const onSubmit = async (data: ForAddProps): Promise<void> => {
    try {
      if (!imageUrl) {
        toast({
          status: 'error',
      ...
      dataWithUrl.url = imageUrl;

      mutation.mutate(dataWithUrl);
